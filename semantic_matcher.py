...
def extract_resume_keywords(text, top_n=30):
    ...
    tokens = [word for word in RegexpTokenizer(r'\w+').tokenize(text)
...
def semantic_match_resume(resume_text, jobs_df, top_n=10):
    ...
    resume_skills = extract_skills_from_text(resume_text, jobs_df["combined_text"])
    jobs_df["matched_skills"], jobs_df["missing_skills"] = zip(
        *jobs_df["combined_text"].apply(lambda txt: get_skill_matches(resume_skills, txt))
    )
